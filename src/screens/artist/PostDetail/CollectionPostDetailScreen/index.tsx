...
import UserCollectionOptionMenu from '~/components/molecules/UserCollectionOptionMenu';
import {queryClient} from '~/graphql/AuthProvider';
import {queryKeys} from '~/constants/queryKeys';
import Video from 'react-native-video';

const CollectionPostDetailScreen = ({route}: {route: any}) => {
  const entityId = route?.params?.entityId;
  const baseCollectionId = route?.params?.baseCollectionId;
  const collectionId = route?.params?.collectionId;

  const {data, isLoading} = useGetPostById(entityId);
  const {showPostModal, setShowPostModal} = usePostModalStore(state => state);
  const {setShowModal} = useCollectionOptionMenuStore(state => state);
...
export default CollectionPostDetailScreen;
